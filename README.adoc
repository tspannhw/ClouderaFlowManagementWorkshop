...
In this virtual hands-on workshop, you will build a real workflow for an **IoT Predictive Maintenance** use case.    You will get hands-on experience in using CFM for production use cases.
...
Pick any REST API of your choice, but I have walked through this one:

https://randomuser.me/api/?format=csv&nat=us&results=25&noinfo
...
* https://w1.weather.gov/xml/current_obs/display.php?stid=KTTN
...
Example query on the random user example data:

[source,sql]
----
SELECT * FROM FLOWFILE
WHERE ("location.state" = 'New Jersey'
OR "location.state" = 'New York'
OR "location.state" = 'Pennsylvania'
OR "location.state" = 'Connecticut')
----

[source,sql]
----
CREATE EXTERNAL TABLE IF NOT EXISTS `people` (`gender` STRING, `name.title` STRING, `name.first` STRING, `name.last` STRING, `location.street.number` BIGINT, `location.street.name` STRING, `location.city` STRING, `location.state` STRING, `location.country` STRING, `location.postcode` BIGINT, `location.coordinates.latitude` DOUBLE, `location.coordinates.longitude` DOUBLE, `location.timezone.offset` STRING, `location.timezone.description` STRING, `email` STRING, `login.uuid` STRING, `login.username` STRING, `login.password` STRING, `login.salt` STRING, `login.md5` STRING, `login.sha1` STRING, `login.sha256` STRING, `dob.date` STRING, `dob.age` BIGINT, `registered.date` STRING, `registered.age` BIGINT, `phone` STRING, `cell` STRING, `id.name` STRING, `id.value` STRING, `picture.large` STRING, `picture.medium` STRING, `picture.thumbnail` STRING, `nat` STRING) 
STORED AS ORC
LOCATION '/tmp/people'
----
== Resources
